...
class Game(ShowBase):
    ...
    def __init__(self):
        ...
        self.model.setPos(-2, -30, 30)

        self.model2 = loader.loadModel('models/environment')
        self.model2.reparentTo(render)
        self.model2.setScale(0.1)
        self.model2.setPos(-2, 25, -3)
        
        # Створюємо NodePath для камери і переміщуємо його в потрібне положення
        self.camera_node = NodePath('FighterTexture.tif')
        self.camera_node.reparentTo(render)
        self.camera_node.setPos(0, -30, 10) 
        
        self.camera_node2 = NodePath('camera_node')
        self.camera_node2.reparentTo(render)
        self.camera_node2.setPos(0, -30, 10) # Встановлюємо камеру позаду об'єкта
        ...
        self.rotation_speed = 2.0  # Швидкість обертання камери
        self.movement_speed = 1.5  # Швидкість переміщення камери
